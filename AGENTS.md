...
- No definir animaciones o interacciones de scroll detalladas antes de C2; cuando se implementen, deben usar JavaScript mínimo, progressive enhancement, fallback funcional sin JS y respetar `prefers-reduced-motion` (Decisión D7). C2 ya cerró esto para el motion editorial de Home: GSAP + ScrollTrigger quedan autorizados como única excepción a "sin dependencias nuevas" (Decisión D112, 2026-09-07) — cualquier otra librería de animación (AOS, Lenis, etc.) sigue sin autorización.
...
- **Git:** repositorio inicializado el 2026-07-17 (`git init`); en commits activos desde el prototipo. Tres ramas quedan conectadas a Cloudflare Pages Builds del proyecto "aaalegal" — `main`, `product-lead-ajustes-visuales` y `product-lead-refinement-v2` — **cualquiera de las tres puede disparar un build y quedar promovida como el despliegue activo**, no solo la que se considere "la de despliegue" en un momento dado (esto causó un incidente real el 2026-09-07: un push a `main` sin las últimas decisiones de otra rama revirtió el sitio en vivo). Por eso, **antes de pushear a cualquiera de esas tres ramas, verificar que las otras dos no tengan commits que esta rama no tenga** (`git log <rama> --oneline` cruzado) — si los tienen, se unifica primero (merge), nunca se pushea a ciegas. Commits solo cuando el Owner los solicite explícitamente; nunca `--force`, `--no-verify`, ni reescritura de historia sin autorización explícita; nunca commitear secretos (`.env`, `wp-config.php`, credenciales) — ver `.gitignore`. Puntos de control de seguridad se guardan como tags anotados (tag vigente: `checkpoint-mvp-unificado-2026-09-07`, sobre el commit unificado `1335235` — reemplaza a `checkpoint-prototype-estable-2026-09-07`, que quedó desactualizado el mismo día que se creó, ver STATUS.md § Nota de unificación de ramas).
- **Política de ramas post-unificación (Decisión D113, 2026-09-07):** las tres ramas se mantienen abiertas indefinidamente (Owner y Product Lead siguen trabajando en paralelo) — no se cierran ni se borran. Pero **`main` debe seguir siendo siempre la versión vigente de https://aaalegal.design-505.workers.dev/**: antes de dar por cerrada cualquier tarea que toque `main`, verificar en vivo que el sitio refleja ese commit (no basta con pushear). Si `product-lead-ajustes-visuales` o `product-lead-refinement-v2` divergen de `main` en el futuro, se unifican hacia `main` antes de considerar cualquiera de las otras dos como la fuente de la versión en producción.
...
- **Dependencias:** no se instala ninguna dependencia (plugin de WordPress, paquete npm/composer) sin autorización explícita del Owner. La elección de stack técnico para WordPress fue autorizada y registrada en D110 (2026-09-07) — cualquier plugin o dependencia fuera de la lista de esa decisión sigue necesitando autorización nueva. GSAP 3.15.0 + ScrollTrigger 3.15.0 (`prototype/js/gsap/`, exclusivos del motion editorial de Home) quedaron autorizados en D112 (2026-09-07): el Owner confirmó que el prototipo actual, ya con esa animación implementada, es la versión aprobada — deja de ser una pregunta pendiente.
...
**Estado actual (2026-09-07): C0 = APROBADO, C1 = APROBADO, copy final = APROBADO (D21), C2 = prototipo estable y unificado (decisiones D1–D111, incluye motion editorial GSAP/ScrollTrigger de Home, verificado en vivo en https://aaalegal.design-505.workers.dev/), 03A — Arquitectura WordPress = CERRADO (D110), dependencia GSAP/ScrollTrigger = CERRADA (D112), política de ramas post-unificación = CERRADA (D113: `main`, `product-lead-ajustes-visuales` y `product-lead-refinement-v2` se mantienen abiertas, `main` sigue siendo la fuente de la versión en producción), implementación WordPress de las 5 páginas reales + Blog = CONSTRUIDA (D114) — pendiente de verificación en vivo por el Owner siguiendo `WORDPRESS-SETUP.md` en Local (este entorno de trabajo no tiene PHP/MySQL para verificarlo).** El prototipo estático vive en [`/prototype/`](prototype/) y ya tiene un checkpoint de seguridad (tag `checkpoint-mvp-unificado-2026-09-07`). Tema propio en `wp-content/themes/aaa-estudio-legal/` con `theme.json`, CSS de componentes, motion GSAP condicional, y un pattern por página con el copy real ya puesto — guía paso a paso completa en `WORDPRESS-SETUP.md`. Sigue pendiente: contenido real del cliente para founders/Talento AAA/casos de éxito y fotografía (todo lo demás sigue como placeholder honesto), la URL externa de ADY, y la migración del contenido aprobado del prototipo estático a los patterns/páginas de WordPress. Ver [STATUS.md](STATUS.md) para el detalle sesión por sesión. Ningún agente autoaprobó nada en nombre de Product Lead o cliente.
