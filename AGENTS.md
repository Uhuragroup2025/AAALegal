...
**Estado actual (2026-09-07): C0 = APROBADO, C1 = APROBADO, copy final = APROBADO (D21), C2 = prototipo estable y unificado (decisiones D1–D111, incluye motion editorial GSAP/ScrollTrigger de Home, verificado en vivo en https://aaalegal.design-505.workers.dev/), 03A — Arquitectura WordPress = CERRADO (D110), dependencia GSAP/ScrollTrigger = CERRADA (D112), política de ramas post-unificación = CERRADA (D113: `main`, `product-lead-ajustes-visuales` y `product-lead-refinement-v2` se mantienen abiertas, `main` sigue siendo la fuente de la versión en producción).** El prototipo estático vive en [`/prototype/`](prototype/) y ya tiene un checkpoint de seguridad (tag `checkpoint-mvp-unificado-2026-09-07`). Arranca ahora la fase de infraestructura WordPress: desarrollo local con la app **Local** (WP Engine), tema propio en `wp-content/themes/aaa-estudio-legal/`, guía paso a paso en `WORDPRESS-SETUP.md` — pendiente de revisión para incorporar el motion/fotografía/Contacto de D99–D109, que el andamiaje actual todavía no refleja (ver STATUS.md). Sigue pendiente: contenido real del cliente para founders/Talento AAA/casos de éxito y fotografía (todo lo demás sigue como placeholder honesto), la URL externa de ADY, y la migración del contenido aprobado del prototipo estático a los patterns/páginas de WordPress. Ver [STATUS.md](STATUS.md) para el detalle sesión por sesión. Ningún agente autoaprobó nada en nombre de Product Lead o cliente.
