...
Decisión de esta fase: **no implementar scrollytelling, sticky-crossfade, pinning ni scroll-jacking**. Desde la primera fase de motion de Home (2026-08-28) sí se admite un parallax fotográfico mínimo y exclusivo del hero en desktop.
...
| Entrada editorial de Home (`data-motion`) | Coordinar hero, H2, paneles de servicios y cifras sin alterar el flujo ni ocultar contenido en el HTML/CSS base | GSAP + ScrollTrigger locales, una sola vez por bloque | Opacidad + `transform`, desplazamientos de 7–24px y stagger de 60–130ms; sin propiedades de layout | Todo permanece visible si GSAP, ScrollTrigger o `home-motion.js` no cargan | `gsap.matchMedia()` entrega inmediatamente los estados finales y no crea timelines |
...
| Parallax muy sutil del hero | Dar profundidad contenida a la fotografía sin competir con el mensaje | ScrollTrigger con `scrub`, únicamente desde 1024px | `yPercent` de −1.4 a 1.4 y escala de 1.045 a 1.06; sin pinning | Sin JS, la imagen conserva su composición estática | No se crea bajo `prefers-reduced-motion: reduce`; tampoco se activa en tablet/mobile |
