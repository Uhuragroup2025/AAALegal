...
| D-07 | Dirección cromática (candidata B, acento frío tecnológico) es una exploración presentada, no una decisión aprobada — ya está aplicada en el CSS como candidata, pero sigue sujeta a validación. *(La parte tipográfica de este hallazgo quedó cerrada: ✅ Decisión D22, Owner, 2026-07-29 — Space Grotesk aprobada para títulos, ver PROJECT.md → Decisiones)* | N/A (es una decisión de diseño pendiente, no un defecto técnico) | No bloquea — es exactamente el objetivo de presentarlo como candidato | Product Lead / cliente (Gate 1) | Abierto — reducido a dirección cromática únicamente |
| D-08 | El mosaico asimétrico de los 4 focos (`.focus-mosaic`) asume exactamente 4 elementos en un orden fijo (spans manuales por `:nth-child`) — no soporta un número variable sin ajustar CSS | Media | No bloquea el prototipo; si el número de focos cambiara sería necesario ajustar el layout | 03A — Arquitectura WordPress | Abierto |
...
- [ ] Portafolio de servicios acotado a los 4 focos acordados (Insolvencia, Concursal, Normalización de Activos, Corporativo) — sin filtración de las demás áreas del sitio actual/brochure
